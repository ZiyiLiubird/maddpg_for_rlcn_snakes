...
def main(args):
    ...
    while episode < args.max_episodes:
        ...
        while True:
            ...
            action = onehot_from_logits(logits).cpu().detach().numpy()
            model.replay_buffer.push(obs, action, step_reward, next_obs, done)
            for agent_i in range(ctrl_agent_num):
                model.update(agent_i)
            ...
            if args.episode_length <= step or (True in done):
                ...
                if model.loss():
                    writer.add_scalars(loss_tag, global_step=episode,
                                       tag_scalar_dict={'actor0': model.agents[0].a_loss, 'critic0': model.agents[0].c_loss,
                                                        'actor1': model.agents[1].a_loss, 'critic1': model.agents[1].c_loss,
                                                        'actor2': model.agents[2].a_loss, 'critic2': model.agents[2].c_loss})

                # if model.c_loss and model.a_loss:
                #     print(f'\t\t\t\ta_loss {model.a_loss:.3f} c_loss {model.c_loss:.3f}')

                if episode % args.save_interval == 0:
                    score = np.sum(episode_reward[0:3])
                    model.save_model(run_dir, episode, score)
                elif (np.sum(episode_reward[0:3]) > max_return) and (np.sum(episode_reward[0:3]) > 17):
                    max_return = np.sum(episode_reward[0:3])
                    score = max_return
                    model.save_model(run_dir, episode, score)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--a_clip', default=1.0, type=float)
    parser.add_argument('--c_clip', default=1.0, type=float)
